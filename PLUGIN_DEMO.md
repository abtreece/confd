...
Start a throwaway PostgreSQL instance:

```bash
docker run -d --name confd-plugin-pg \
...
```bash
docker exec -i confd-plugin-pg \
  psql -U admin -d confd < cmd/confd-plugin-postgres/setup.sql
```
...
```bash
docker exec -i confd-plugin-pg psql -U admin -d confd -c "
