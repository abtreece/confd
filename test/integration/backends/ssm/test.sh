...
confd ssm --onetime --log-level debug --confdir ./test/integration/shared/confdir --interval 5

# Run confd with --watch, expecting it to fail (watch not supported)
if confd ssm --onetime --log-level debug --confdir ./test/integration/shared/confdir --interval 5 --watch 2>/dev/null; then
...
if confd ssm --onetime --log-level debug --confdir ./test/integration/shared/confdir --interval 5 2>/dev/null; then
