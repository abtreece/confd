...
confd dynamodb --onetime --log-level debug --confdir ./test/integration/shared/confdir --interval 5 --table confd

# Run confd with --watch, expecting it to fail (watch not supported)
if confd dynamodb --onetime --log-level debug --confdir ./test/integration/shared/confdir --interval 5 --table confd --watch 2>/dev/null; then
...
if confd dynamodb --onetime --log-level debug --confdir ./test/integration/shared/confdir --interval 5 --table confd 2>/dev/null; then
