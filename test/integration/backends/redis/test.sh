...
confd redis --onetime --log-level debug --confdir ./test/integration/shared/confdir --interval 5 --node "$REDIS_HOST:$REDIS_PORT"

# Test with host:port/db format
confd redis --onetime --log-level debug --confdir ./test/integration/shared/confdir --interval 5 --node "$REDIS_HOST:$REDIS_PORT/0"
