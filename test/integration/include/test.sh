...
./bin/confd env --onetime --log-level error --confdir ./test/integration/include/confdir
...
if diff -u ./test/integration/include/expect/include.conf /tmp/confd-include-test.conf; then
  echo "PASS: Content matches expected output"
else
  echo "FAIL: Content does not match expected output"
  echo "Expected:"
  cat ./test/integration/include/expect/include.conf
  echo ""
  echo "Actual:"
