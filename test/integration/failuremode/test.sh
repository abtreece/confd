...
cleanup
./bin/confd env --onetime --log-level error --confdir ./test/integration/failuremode/confdir --failure-mode best-effort 2>&1 || true
...
if ./bin/confd env --onetime --log-level error --confdir ./test/integration/failuremode/confdir --failure-mode fail-fast 2>&1; then
