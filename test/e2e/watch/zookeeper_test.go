...
// TestZookeeperWatch_BasicChange verifies that the WatchProcessor detects
// a single key change in Zookeeper and updates the output file.
func TestZookeeperWatch_BasicChange(t *testing.T) {
	// Use separate contexts for container lifecycle and processor
	containerCtx := context.Background()

	// Setup Zookeeper container
	container := containers.NewZookeeperContainer()
	...
	// Set initial value in Zookeeper
...
`, destPath)
	...
	if err := testenv.WaitForFile(t, destPath, 15*time.Second, "value=initial"); err != nil {
		t.Fatalf("Initial output not created: %v", err)
	}

	// Update value in Zookeeper
	time.Sleep(500 * time.Millisecond) // Allow watch to settle
	if err := env.SetBackendValue(containerCtx, "/test/value", "updated"); err != nil {
		t.Fatalf("Failed to update value: %v", err)
	}

	// Wait for updated output
	content, err := testenv.WaitForFileUpdate(t, destPath, 15*time.Second, "value=initial")
	if err != nil {
...
// TestZookeeperWatch_MultipleUpdates verifies that the WatchProcessor correctly
// handles multiple sequential updates to the same key.
func TestZookeeperWatch_MultipleUpdates(t *testing.T) {
	// Use separate contexts for container lifecycle and processor
	containerCtx := context.Background()
...
`, destPath)
	...
	// Wait for initial output
	if err := testenv.WaitForFile(t, destPath, 15*time.Second, "count=0"); err != nil {
		t.Fatalf("Initial output not created: %v", err)
	}

	// Update multiple times
	for i := 1; i <= 3; i++ {
		time.Sleep(500 * time.Millisecond) // Allow watch to detect previous change
		...
		content, err := testenv.WaitForFileUpdate(t, destPath, 15*time.Second, previousValue)
		if err != nil {
...
// TestZookeeperWatch_MultipleKeys verifies that the WatchProcessor correctly
// handles templates that watch multiple keys.
func TestZookeeperWatch_MultipleKeys(t *testing.T) {
	// Use separate contexts for container lifecycle and processor
	containerCtx := context.Background()
...
port=5432`
	if err := testenv.WaitForFile(t, destPath, 15*time.Second, expectedInitial); err != nil {
		t.Fatalf("Initial output not created: %v", err)
	}
	...
	// Wait for update
	content, err := testenv.WaitForFileUpdate(t, destPath, 15*time.Second, expectedInitial)
	if err != nil {
...
// TestZookeeperWatch_GracefulShutdown verifies that the WatchProcessor
// shuts down gracefully when the context is cancelled.
func TestZookeeperWatch_GracefulShutdown(t *testing.T) {
	...
	// Set initial value
	if err := env.SetBackendValue(containerCtx, "/shutdown/value", "graceful"); err != nil {
		t.Fatalf("Failed to set initial value: %v", err)
	}

	// Create template
	env.WriteTemplate("shutdown.tmpl", `value={{ getv "/shutdown/value" }}`)
...
`, destPath)
	...
	// Create and start watch processor
	processor, _, doneChan, _, err := env.CreateWatchProcessor(processorCtx)
	if err != nil {
		t.Fatalf("Failed to create watch processor: %v", err)
	}
	...
	// Wait for initial output
	if err := testenv.WaitForFile(t, destPath, 15*time.Second, "value=graceful"); err != nil {
		t.Fatalf("Initial output not created: %v", err)
	}
	...
	// Wait for processor to stop gracefully
	select {
	case <-doneChan:
